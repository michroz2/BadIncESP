...
### Описание проекта
**BadIncESP** — это высокоточная двухмодульная беспроводная система измерения и визуализации углов наклона (инклинометр). Проект разработан для работы в реальном времени с минимальными задержками и состоит из передатчика (датчика) и приемника (индикатора), связанных по протоколу Bluetooth Low Energy (BLE).

Система не только транслирует основной угол крена, но и включает встроенный математический фильтр (динамическое демпфирование), который автоматически гасит показания крена, если устройство чрезмерно наклоняется вперед или назад.
...
* **Память NVS (Preferences):** Все пользовательские настройки (яркость, чувствительность, реверс ленты) и калибровочные нули осей сохраняются в энергонезависимую память обоих устройств. Запись отложена на 15 секунд после последнего действия пользователя для защиты флэш-памяти от износа.
